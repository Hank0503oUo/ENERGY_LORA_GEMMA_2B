"""Shared config for harness_v02 router-strict LoRA data pipeline.
...
DRIVE_ROOT = Path("/content/drive/MyDrive/energy_lora_router_v02")
...
TOOLS: list[tuple[str, str]] = [
    ...
    ("map_energy_semantics",            "查詢建築在系統中的正式編號/ID（building_id / meter_id / 系統ID/正式編號/設備代碼）；也包含將建築/電表/感測器欄位對應到語意標籤；不是一般用電查詢、不是資料來源清單"),
]
...
BOUNDARY_RULES: list[str] = [
    ...
    "detect_energy_anomalies 是自動偵測異常（突波/停機/漂移）；diagnose_energy_anomaly 是深入診斷單一事件的因果。先偵測再診斷；若問『為什麼異常/原因是什麼』選 diagnose。",
    "validate_strategy_openbse 用 OpenBSE 驗證已採用策略的節能效果；run_openbse_hybrid_counterfactual 是跑假設情境模擬；openbse_hvac_breakdown 是拆解 HVAC 各組件能耗。三者不同層次。",
    "get_top_energy_buildings 是單一年度用電排行（top N）；rank_energy_buildings_across_years 是跨年度跨建築排名。有『排行/top/最高/最低』但只看單年 → get_top；有『跨年/歷年排名』→ rank。",
    "calibrate_sensitivity 是主動校準（回灌/調整/更新模型參數）；get_sensitivity_status 是被動查詢（分析/報告/紀錄/查詢/數據/解析）。『目前的校準狀態/數據/報告』→ get_sensitivity_status。",
    "record_strategy 是記錄已採用策略（動作）；confirm_strategy_adoption 是確認是否已採用（yes/no）；check_strategy_status 是查詢目前狀態。『目前/在不在/實施中』→ check；『確認/用了沒/已採用』→ confirm。",
]
