"""v07 LoRA training script for Colab — boundary repair round.
...
Usage (from Colab):
    export DRIVE_PROJECT_DIR=/content/drive/MyDrive/energy_lora_router_v08
...
def _load_root_config() -> dict[str, Any]:
    candidates = [
        DRIVE_PROJECT_DIR / "v8_training_config.json",
        Path(__file__).resolve().parent.parent / "v8_training_config.json",
    ]
...
OUTPUT_DIR = DRIVE_PROJECT_DIR / "outputs" / str(ROOT_CONFIG.get("output_name", "gemma_dispatch_v08"))

TRAIN_FILE = DRIVE_PROJECT_DIR / str(ROOT_CONFIG.get("train_file", "data/processed/train_v08_dispatch.jsonl"))
VAL_FILE = DRIVE_PROJECT_DIR / str(ROOT_CONFIG.get("val_file", "data/processed/val_v08_dispatch.jsonl"))
SMOKE_FILE = DRIVE_PROJECT_DIR / str(ROOT_CONFIG.get("smoke_file", "data/processed/smoke_v08_dispatch.jsonl"))
MANIFEST_FILE = DRIVE_PROJECT_DIR / str(ROOT_CONFIG.get("manifest_file", "data/processed/harness_v08_manifest.json"))
...
def _save_training_manifest(training_args) -> None:
    ...
    manifest = {
        "project": "energy_lora_router_v08",
        "base_model": MODEL_ID,
