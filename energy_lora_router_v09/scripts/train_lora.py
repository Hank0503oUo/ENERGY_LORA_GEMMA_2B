"""v09 LoRA training script for Colab - format and no_evidence repair round.

Key changes for v09:
- 3-layer schema: dispatch_type / workflow_id / tool completely separated
- no_evidence is routed before actionable tool decisions
- workflow_id placeholder wording removed from the prompt
- focused contrast samples for no_evidence, clarify_needed, and workflow_chain
...
_LOCAL_PROJECT_DIR = Path(__file__).resolve().parent.parent
_COLAB_PROJECT_DIR = Path("/content/drive/MyDrive/energy_lora_router_v09")
_DEFAULT_PROJECT_DIR = _COLAB_PROJECT_DIR if _COLAB_PROJECT_DIR.parent.exists() else _LOCAL_PROJECT_DIR
DRIVE_PROJECT_DIR = Path(os.getenv("DRIVE_PROJECT_DIR", str(_DEFAULT_PROJECT_DIR)))


def _load_root_config() -> dict[str, Any]:
    candidates = [
        DRIVE_PROJECT_DIR / "v9_training_config.json",
        Path(__file__).resolve().parent.parent / "v9_training_config.json",
    ]
...
def _cfg(section: str, key: str, default: Any) -> Any:
    value = ROOT_CONFIG.get(section, {}).get(key, default)
    return default if value is None else value


DATA_DIR = DRIVE_PROJECT_DIR / "data" / "processed"
OUTPUT_DIR = DRIVE_PROJECT_DIR / "outputs" / str(ROOT_CONFIG.get("output_name", "gemma_dispatch_v09"))

TRAIN_FILE = DRIVE_PROJECT_DIR / str(ROOT_CONFIG.get("train_file", "data/processed/train_v09_dispatch.jsonl"))
VAL_FILE = DRIVE_PROJECT_DIR / str(ROOT_CONFIG.get("val_file", "data/processed/val_v09_dispatch.jsonl"))
SMOKE_FILE = DRIVE_PROJECT_DIR / str(ROOT_CONFIG.get("smoke_file", "data/processed/smoke_v09_dispatch.jsonl"))
MANIFEST_FILE = DRIVE_PROJECT_DIR / str(ROOT_CONFIG.get("manifest_file", "data/processed/harness_v09_manifest.json"))

MODEL_ID = os.getenv("MODEL_ID", str(ROOT_CONFIG.get("base_model", "google/gemma-4-e2b-it")))
EXPERIMENT_NAME = os.getenv("EXPERIMENT_NAME", "gemma-e2b-agent-dispatch-v09-r32-bf16-1ep")
GGUF_BASENAME = os.getenv("GGUF_BASENAME", "gemma-e2b-it-agent-dispatch-v09-r32-Q4_K_M.gguf")

MAX_SEQ_LENGTH = int(os.getenv("MAX_SEQ_LENGTH", str(_cfg("training", "max_seq_length", 2048))))
LOAD_IN_4BIT = os.getenv("LOAD_IN_4BIT", str(_cfg("precision", "load_in_4bit", False))).lower() in {"1", "true", "yes"}
LORA_R = int(os.getenv("LORA_R", str(_cfg("lora", "r", 32))))
LORA_ALPHA = int(os.getenv("LORA_ALPHA", str(_cfg("lora", "alpha", LORA_R * 2))))
LORA_DROPOUT = float(os.getenv("LORA_DROPOUT", str(_cfg("lora", "dropout", 0))))

NUM_TRAIN_EPOCHS = float(os.getenv("NUM_TRAIN_EPOCHS", str(_cfg("training", "num_train_epochs", 1))))
TRAIN_BATCH_SIZE = int(os.getenv("TRAIN_BATCH_SIZE", str(_cfg("training", "per_device_train_batch_size", 8))))
GRAD_ACCUM_STEPS = int(os.getenv("GRAD_ACCUM_STEPS", str(_cfg("training", "gradient_accumulation_steps", 4))))
LEARNING_RATE = float(os.getenv("LEARNING_RATE", str(_cfg("training", "learning_rate", 3e-5))))
...
WARMUP_RATIO = float(os.getenv("WARMUP_RATIO", str(_cfg("training", "warmup_ratio", 0.03))))
SAVE_STEPS = int(os.getenv("SAVE_STEPS", str(_cfg("training", "save_steps", 40))))
EVAL_STEPS = int(os.getenv("EVAL_STEPS", str(_cfg("training", "eval_steps", 40))))
SAVE_TOTAL_LIMIT = int(os.getenv("SAVE_TOTAL_LIMIT", str(_cfg("training", "save_total_limit", 3))))
...
def _is_dispatch_dataset_row(item: dict[str, Any]) -> bool:
...
def validate_data() -> None:
    ...
    sample_rows = (train_rows[:3] + val_rows[:3] + smoke_rows[:3])
    dispatch_mode = (
        manifest.get("profile") == "agent_dispatch_training"
        or any(_is_dispatch_dataset_row(row) for row in sample_rows)
    )

    if dispatch_mode:
        if manifest.get("profile") not in {"agent_dispatch_training", "dispatch"}:
            print(f"[validate] INFO: forcing dispatch mode from row schema (manifest profile={manifest.get('profile')!r})")
    else:
    ...
    for split_name, rows in [("train", train_rows), ("val", val_rows), ("smoke", smoke_rows)]:
        for idx, item in enumerate(rows, 1):
            ...
            if dispatch_mode:
                expected_dispatch = item.get("expected_dispatch_type", "")
                expected_workflow = item.get("expected_workflow_id", "")
                expected_answerability = item.get("expected_answerability", "")

                if parsed.get("dispatch_type") != expected_dispatch:
                    raise ValueError(
                        f"{split_name}:{idx} expected_dispatch_type mismatch: "
                        f"{expected_dispatch} != {parsed.get('dispatch_type')}"
                    )
                if expected_workflow and parsed.get("workflow_id") != expected_workflow:
                    raise ValueError(
                        f"{split_name}:{idx} expected_workflow_id mismatch: "
                        f"{expected_workflow} != {parsed.get('workflow_id')}"
                    )
                if expected_answerability and parsed.get("answerability") != expected_answerability:
                    raise ValueError(
                        f"{split_name}:{idx} expected_answerability mismatch: "
                        f"{expected_answerability} != {parsed.get('answerability')}"
                    )
            else:
    ...
    mode_name = "dispatch" if dispatch_mode else "router"
    print(f"  manifest: {manifest['version']} {manifest.get('profile')} {manifest['total']} ({mode_name})")
...
def _save_training_manifest(training_args) -> None:
    ...
    manifest = {
        "project": "energy_lora_router_v09",
        "base_model": MODEL_ID,
        ...
        "training": {
            ...
            "load_in_4bit": LOAD_IN_4BIT,
            "precision_note": "bf16 if supported, otherwise fp16; v09 default is no 4-bit base",
        },
...
def main() -> None:
    ...
    print("=" * 60)
    print(f"v09 Training Pipeline | {EXPERIMENT_NAME}")
    print("=" * 60)
