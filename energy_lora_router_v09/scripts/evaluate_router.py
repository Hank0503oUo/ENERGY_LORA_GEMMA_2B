"""v09 router evaluator - format and no_evidence repair round.
...
OUTPUT_DIR = DRIVE_PROJECT_DIR / "outputs" / "gemma_dispatch_v09" / "eval"
...
DATA_YEAR_MIN = int(os.getenv("DATA_YEAR_MIN", "2017"))
DATA_YEAR_MAX = int(os.getenv("DATA_YEAR_MAX", "2023"))

_UNSAFE_RULES: list[tuple[_re.Pattern, str]] = [
    (_re.compile(r"全[部力]開|開到最[大高強]|強制.*開|強制.*設定|手動全開"), "要求強制控制設備"),
    (_re.compile(r"偽造|竄改|捏造|修改.*文件|修改.*記錄|修改.*數據|隱藏.*文件|隱藏.*法規"), "要求偽造或竄改資料"),
    (_re.compile(r"刪除|刪掉|關[掉閉]所有|停掉.*系統|取消.*上限|還原.*出廠|破壞"), "要求破壞或移除設備"),
    (_re.compile(r"隨便給|隨便查|改低一?點|改漂亮|把.*數字改"), "要求捏造或篡改數據"),
]

_UNSUPPORTED_SCOPE_RULES: list[tuple[_re.Pattern, str]] = [
    (_re.compile(r"天然氣|瓦斯|水費|電費帳單|碳足跡|碳排放|碳排|停車"), "系統無此類資料"),
    (_re.compile(r"台大醫院|醫院|診所|醫學中心|附設醫院"), "校外或醫療機構不在校園能源範圍"),
    (_re.compile(r"交大|清大|師大|成大|政大|中央大學|中山大學|中興|中正大學|陽明交大"), "非台大校園範圍"),
    (_re.compile(r"教授.*用電|個人用電|私人|校長.*用電|院長.*用電|實驗室.*用電|宿舍.*個人"), "隱私或敏感查詢"),
]

_CLARIFY_RULES: list[tuple[_re.Pattern, str]] = [
    (_re.compile(r"^(然後呢|這個|那個|那棟|看一下|幫我查一下|請問)\s*$"), "ambiguous_reference"),
    (_re.compile(r"^(去年的用電|節能方法|用電狀況|異常|幫我看一下)\s*$"), "missing_required_arguments"),
]

# Secondary clarify check: query is very short and has no building or year
_SHORT_AMBIGUOUS_RE = _re.compile(r"^.{1,5}$")


_YEAR_RE = _re.compile(r"(?:19|20)\d{2}")
_ENERGY_QUERY_RE = _re.compile(r"用電|耗電|電力|能源|EUI|kWh|kw|peak|節能|熱點|改善|策略|HVAC|空調|照明")
_BUILDING_HINT_RE = _re.compile(r"<[^>]*BUILDING[^>]*>|BUILDING_[A-Z]|LIBRARY_BUILDING|DORM_BUILDING|[\u4e00-\u9fff]{1,16}(館|樓|棟|院|中心)")


def _extract_years(user_input: str) -> list[int]:
    return [int(match.group(0)) for match in _YEAR_RE.finditer(user_input)]


def _extract_buildings(user_input: str) -> list[str]:
    buildings: list[str] = []
    for match in _BUILDING_HINT_RE.finditer(user_input):
        value = match.group(0)
...
def _dispatch_dict(
    ...
    *,
    ...
    return {
        "tool": "__refusal__" if dispatch_type == "refusal" else "",
        ...
        "_pre_gate": True,
        "_pre_gate_type": answerability,
    }


def run_pre_gate(user_input: str) -> dict[str, Any] | None:
    """Return a deterministic dispatch dict when a boundary is obvious."""
    for pattern, reason in _UNSAFE_RULES:
        if pattern.search(user_input):
            result = _dispatch_dict("refusal", "none", "unsafe_operation", reason=reason)
            result["stop_conditions"] = ["if_unsafe_operation_refuse_without_tool"]
            return result

    for pattern, reason in _UNSUPPORTED_SCOPE_RULES:
        if pattern.search(user_input):
            result = _dispatch_dict("refusal", "none", "unsupported_scope", reason=reason)
            result["stop_conditions"] = ["if_unsupported_scope_refuse_without_tool"]
            return result

    years = _extract_years(user_input)
    out_of_range_years = [y for y in years if y < DATA_YEAR_MIN or y > DATA_YEAR_MAX]
    buildings = _extract_buildings(user_input)
    if out_of_range_years and buildings and _ENERGY_QUERY_RE.search(user_input):
        result = _dispatch_dict(
            "no_evidence",
            "none",
            "no_evidence_expected",
            reason=f"year outside supported range {DATA_YEAR_MIN}-{DATA_YEAR_MAX}",
            buildings=buildings,
            years=out_of_range_years,
        )
        result["stop_conditions"] = ["if_energy_values_missing_report_missing_not_zero"]
        return result

    for pattern, answerability in _CLARIFY_RULES:
        if pattern.search(user_input):
            ...
            result = _dispatch_dict("clarify_needed", "none", answerability)
            result["stop_conditions"] = [stop]
            return result

    # Secondary: catch very short queries with no building/year context
    if _SHORT_AMBIGUOUS_RE.match(user_input) and not _extract_buildings(user_input) and not _extract_years(user_input):
        result = _dispatch_dict("clarify_needed", "none", "missing_required_arguments")
        result["stop_conditions"] = ["if_required_arguments_missing_ask_clarification"]
        return result
        ...
        if ans:
            ...
            if dispatch_mode:
                ...
                if is_non_answerable:
                    ...
                    if predicted == expected and predicted_ans == ans:
...
def check_gates(report: dict[str, Any]) -> dict[str, bool]:
    """Check v09 eval gates. Returns gate_name -> passed bool."""
    if report.get("schema_mode") == "dispatch":
        by_diff = report.get("by_difficulty", {})
        by_dispatch = report.get("by_expected_dispatch_type", {}) or report.get("by_expected_tool", {})
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate v09 router adapter")
    ...
    parser.add_argument("--format-smoke-file", type=Path, help="Format smoke test JSONL")
    ...
    parser.add_argument("--prefix", type=str, default="v09", help="Output file prefix")
