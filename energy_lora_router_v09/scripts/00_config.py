"""Shared config for harness router/dispatch LoRA data pipeline (v05 edition).
...
This is a copy of the v04 config with paths re-pointed at v05.
Tool catalogue + BOUNDARY_RULES are kept stable unless the v05 dataset
converter intentionally changes the output schema.
...
DRIVE_ROOT = Path("/content/drive/MyDrive/energy_lora_router_v05")
...
TRAIN_FILE = DATA_DIR / "train_v05.jsonl"
VAL_FILE = DATA_DIR / "val_v05.jsonl"
SMOKE_FILE = DATA_DIR / "smoke_v05.jsonl"
...
BOUNDARY_RULES: list[str] = [
    ...
    "校準敏感度(calibrate_sensitivity) 是進階維護工具，僅在使用者提供預測節能量 vs 實際節能量（含數字）及主因時才可使用。僅說「幫我校準模型」而無具體數字者，應回覆 __refusal__ 並要求提供參數。",]
