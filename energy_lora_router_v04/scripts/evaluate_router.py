...
OUTPUT_DIR = DRIVE_PROJECT_DIR / "outputs" / "gemma_router_strict_v04" / "eval"
...
def evaluate_split(
    ...
    for item, raw in zip(rows, raw_outputs):
        ...
        if ans:
            ...
            is_non_answerable = ans in (
                "unsupported_scope", "unsupported_capability",
                "unsafe_operation", "ambiguous_need_clarification", "missing_data",
            )

            # Answerability correctness
            if is_non_answerable and predicted == "__refusal__":
                answerability_correct += 1
                non_answerable_refusal_correct += 1
            elif not is_non_answerable and predicted != "__refusal__":
                answerability_correct += 1
    ...
    report = {
        "prefix": prefix,
        "model_path": getattr(model, "__v04_label", "unknown"),
...
def compute_selection_score(report: dict[str, Any]) -> float:
    """Weighted selection score per v04 spec.
...
def check_gates(report: dict[str, Any]) -> dict[str, bool]:
    """Check v04 eval gates. Returns gate_name -> passed bool."""
    by_diff = report.get("by_difficulty", {})
    by_tool = report.get("by_expected_tool", {})
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate v04 router adapter")
    ...
    parser.add_argument("--prefix", type=str, default="v04", help="Output file prefix")
    ...
    if args.adapter and not args.base_model_only:
        ...
        model.__v04_label = str(args.adapter)
    else:
        model.__v04_label = "base_model"
