"""v07 LoRA training script for Colab — schema clarity round.

Key changes from v06:
- 3-layer schema: dispatch_type / workflow_id / tool completely separated
- no_evidence forced to required_tools=[]
- Contrast samples for single_tool vs workflow_chain
- JSON grammar for constrained decoding

Usage (from Colab):
    export DRIVE_PROJECT_DIR=/content/drive/MyDrive/energy_lora_router_v07
...
DRIVE_PROJECT_DIR = Path(os.getenv("DRIVE_PROJECT_DIR", "/content/drive/MyDrive/energy_lora_router_v07"))


def _load_root_config() -> dict[str, Any]:
    candidates = [
        DRIVE_PROJECT_DIR / "v7_training_config.json",
        Path(__file__).resolve().parent.parent / "v7_training_config.json",
    ]
...
OUTPUT_DIR = DRIVE_PROJECT_DIR / "outputs" / str(ROOT_CONFIG.get("output_name", "gemma_dispatch_v07"))

TRAIN_FILE = DRIVE_PROJECT_DIR / str(ROOT_CONFIG.get("train_file", "data/processed/train_v07_dispatch.jsonl"))
VAL_FILE = DRIVE_PROJECT_DIR / str(ROOT_CONFIG.get("val_file", "data/processed/val_v07_dispatch.jsonl"))
SMOKE_FILE = DRIVE_PROJECT_DIR / str(ROOT_CONFIG.get("smoke_file", "data/processed/smoke_v07_dispatch.jsonl"))
MANIFEST_FILE = DRIVE_PROJECT_DIR / str(ROOT_CONFIG.get("manifest_file", "data/processed/harness_v07_manifest.json"))

MODEL_ID = os.getenv("MODEL_ID", str(ROOT_CONFIG.get("base_model", "google/gemma-4-e2b-it")))
EXPERIMENT_NAME = os.getenv("EXPERIMENT_NAME", "gemma-e2b-agent-dispatch-v07-r32-bf16-1ep")
GGUF_BASENAME = os.getenv("GGUF_BASENAME", "gemma-e2b-it-agent-dispatch-v07-r32-Q4_K_M.gguf")
...
def _save_training_manifest(training_args) -> None:
    ...
    manifest = {
        "project": "energy_lora_router_v07",
        "base_model": MODEL_ID,
        ...
        "training": {
            ...
            "load_in_4bit": LOAD_IN_4BIT,
            "precision_note": "bf16 if supported, otherwise fp16; v07 default is no 4-bit base",
        },
...
def main() -> None:
    ...
    print("=" * 60)
    print(f"v07 Training Pipeline | {EXPERIMENT_NAME}")
    print("=" * 60)
