"""v05 LoRA training script for Colab.

Key changes for v05:
- Agent dispatch training target: workflow / answerability / entity lock.
- bf16 LoRA by default (`LOAD_IN_4BIT=false`) for cleaner final adapters.
- 1 epoch to reduce overfitting.
- lr=3e-5 for a conservative schema-change run.
- save_strategy=steps (every 40 steps) for checkpoint selection
- load_best_model_at_end=True with eval_loss metric
- Supports v05 processed data generated from DeepSeek dispatch samples.

Usage (from Colab):
    export DRIVE_PROJECT_DIR=/content/drive/MyDrive/energy_lora_router_v05
...
DRIVE_PROJECT_DIR = Path(os.getenv("DRIVE_PROJECT_DIR", "/content/drive/MyDrive/energy_lora_router_v05"))


def _load_root_config() -> dict[str, Any]:
    candidates = [
        DRIVE_PROJECT_DIR / "v5_training_config.json",
        Path(__file__).resolve().parent.parent / "v5_training_config.json",
    ]
...
OUTPUT_DIR = DRIVE_PROJECT_DIR / "outputs" / str(ROOT_CONFIG.get("output_name", "gemma_dispatch_v05"))

TRAIN_FILE = DRIVE_PROJECT_DIR / str(ROOT_CONFIG.get("train_file", "data/processed/train_v05.jsonl"))
VAL_FILE = DRIVE_PROJECT_DIR / str(ROOT_CONFIG.get("val_file", "data/processed/val_v05.jsonl"))
SMOKE_FILE = DRIVE_PROJECT_DIR / str(ROOT_CONFIG.get("smoke_file", "data/processed/smoke_v05.jsonl"))
MANIFEST_FILE = DRIVE_PROJECT_DIR / str(ROOT_CONFIG.get("manifest_file", "data/processed/harness_v05_manifest.json"))

MODEL_ID = os.getenv("MODEL_ID", str(ROOT_CONFIG.get("base_model", "google/gemma-4-e2b-it")))
EXPERIMENT_NAME = os.getenv("EXPERIMENT_NAME", "gemma-e2b-agent-dispatch-v05-r32-bf16-1ep")
GGUF_BASENAME = os.getenv("GGUF_BASENAME", "gemma-e2b-it-agent-dispatch-v05-r32-Q4_K_M.gguf")
...
def _save_training_manifest(training_args) -> None:
    ...
    manifest = {
        "project": "energy_lora_router_v05",
        "base_model": MODEL_ID,
        ...
        "training": {
            ...
            "load_in_4bit": LOAD_IN_4BIT,
            "precision_note": "bf16 if supported, otherwise fp16; v05 default is no 4-bit base",
        },
...
def main() -> None:
    ...
    print("=" * 60)
    print(f"v05 Training Pipeline | {EXPERIMENT_NAME}")
    print("=" * 60)
