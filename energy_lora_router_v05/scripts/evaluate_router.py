...
OUTPUT_DIR = DRIVE_PROJECT_DIR / "outputs" / "gemma_dispatch_v05" / "eval"
...
def check_gates(report: dict[str, Any]) -> dict[str, bool]:
    """Check v05 eval gates. Returns gate_name -> passed bool."""
    if report.get("schema_mode") == "dispatch":
        by_diff = report.get("by_difficulty", {})
        by_dispatch = report.get("by_expected_dispatch_type", {}) or report.get("by_expected_tool", {})
        ...
        gates = {
            ...
            "workflow_accuracy_85": (report.get("workflow_accuracy", 0) or 0) >= 0.85,
            "single_tool_accuracy_90": _get_acc(by_dispatch.get("single_tool", {})) >= 0.90,
            "workflow_chain_accuracy_85": _get_acc(by_dispatch.get("workflow_chain", {})) >= 0.85,
            "clarify_accuracy_90": _get_acc(by_dispatch.get("clarify_needed", {})) >= 0.90,
            "no_evidence_accuracy_90": _get_acc(by_dispatch.get("no_evidence", {})) >= 0.90,
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate v05 router adapter")
    ...
    parser.add_argument("--prefix", type=str, default="v05", help="Output file prefix")
