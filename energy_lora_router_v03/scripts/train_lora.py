"""v03 LoRA training script for Colab.

Key changes from v02:
- 2 epochs (not 4) to avoid overfit
- rank=32, alpha=64 (more capacity)
- lr=1e-4 (conservative)
- load_best_model_at_end=True
- Saves manifest with selection metadata
- Supports patch dataset merging

Usage (from Colab):
    export DRIVE_PROJECT_DIR=/content/drive/MyDrive/energy_lora_router_v03
...
OUTPUT_DIR = DRIVE_PROJECT_DIR / "outputs" / "gemma_router_strict_v03"
...
EXPERIMENT_NAME = os.getenv("EXPERIMENT_NAME", "gemma-e2b-energy-router-v03-r32")
GGUF_BASENAME = os.getenv("GGUF_BASENAME", "gemma-e2b-it-energy-router-v03-r32-Q4_K_M.gguf")
...
# v03: warmup_ratio instead of fixed steps
...
def _save_training_manifest(training_args) -> None:
    ...
    manifest = {
        "project": "energy_lora_router_v03",
        "base_model": MODEL_ID,
...
def main() -> None:
    ...
    print("=" * 60)
    print(f"v03 Training Pipeline | {EXPERIMENT_NAME}")
    print("=" * 60)
